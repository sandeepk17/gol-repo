    ...
    stage('triggerdowstream') {
        steps {
            ...
            script{
                ...
                currentBuild.description += "<a href='http://localhost:8080/job/gof-pipeline/'><img src='http://localhost:8080/job/gof-pipeline/badge/icon?config=smoketest'></a>" + "\n"
    ...
    stage('triggerdowstream1') {
        steps {
            ...
            script{
                ...
                currentBuild.description += "<a href='http://192.168.0.100:8080/job/badgetest/'><img src='http://192.168.0.100:8080/job/badgetest/badge/icon?config=integrationtest'></a>" + "\n"                
                //currentBuild.description += '<a href=' + build_res.absoluteUrl +' style="color:' + color + '">build#'+ build_res.number + '</a><br>' + "\n"
                //buildno = "" + build_res.number
            }
            //withCredentials([string(credentialsId: 'OctopusAPIkey', variable: 'APIKey')]) {
